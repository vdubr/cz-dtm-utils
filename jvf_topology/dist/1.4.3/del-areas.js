 ...
 */
export function checkDelAreaContainsDefBodPlocha(dtm) {
    const delOblasti = dtm.doprovodneInformace?.oblastiKompletniZPS.filter((o) => o.typ === 'DEL' && o.plocha !== undefined);
    if (delOblasti === undefined || delOblasti.length === 0)
        return [];
    const errors = [];
    ...
    for (const defbodElementName of defbodTypes) {
        ...
        for (const zaznam of defbodTyp.zaznamy) {
            ...
            errors.push(mkError({
                ...
                geometryIndex: 0,
            }, 'warning', 'DEL_AREA_CONTAINS_DEFBOD_PLOCHA', `Definiční bod leží uvnitř oblasti DEL — po přijetí dojde ke zmenšení oblasti kompletní ZPS.`));
        }
    }
    return errors;
}
//# sourceMappingURL=del-areas.js.map